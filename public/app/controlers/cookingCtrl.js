angular.module('cookingController', ['cookServices'])
    .controller("cookingCtrl", function(Dashboard, Cook, Perso, Auth, User,$timeout,$scope,$window,$interval,$location) {
        var app = this
        ...
        qty = function(e){
            app.err_msg=""
            Cook.getRecipe(e.params.data.text).then(function(data) {
                ...
                for (var i = 0; i < qt.length; i++) {
                    ...
                    for (var j = 0; j < 3; j++) {
                        ...
                        if (j == 0) {//0
                            ...
                            //,&quot;'+ratioEl+'&quot;
                            td.innerHTML = '<div class="ing" onmouseover="displayIngs(&quot;'+ing[i]+'&quot;,2)" onclick="ingToRecipe(&quot;'+ing[i]+'&quot;)" style="cursor:pointer">'+'<img src="assets/img/recipes/'+ing[i]+'.png" height="25px" width="25px">'+'<div id="'+ing[i]+'" class="tooltiptext"></div></div>'
                        } else {//1
                ...
                tr.appendChild(td);
                td=document.createElement("th")
                //ratioEl = parseInt(tr.childNodes[1].innerHTML)
                td.innerHTML='<div class="ing" onmouseover="displayIngs(&quot;'+rec.Recipe+'&quot;,2)" onclick="ingToRecipe(&quot;'+rec.Recipe+'&quot;)" style="cursor:pointer">'+'<img src="assets/img/recipes/'+rec.Recipe+'.png" height="25px" width="25px">'+'<div id="'+rec.Recipe+'" class="tooltiptext"></div></div>'
                td.setAttribute("style", 'text-align:center;');
                tr.appendChild(td)
                tbody.appendChild(tr);
...
/********************************************format selects********/
        var formatRecipe = function(recipe) {
            ...
            var baseUrl = "/assets/img/recipes";
            var $recipe = $(
        ...
        var formatChars = function(char) {
            ...
            var $char = $(
                '<span><img src="' + baseUrl + '/' + classe + '.png" class="img-flag" width="25px" height="25px" style="filter: invert(100%);"/> &emsp;' + char.text + '&emsp;<i class="fa fa-angle-double-right" aria-hidden="true"></i> &emsp;with : <img width="25px" height="25px" src="assets/img/clothes/cook.png">' + clth + '% bonus Exp</span>'
            );
...
/******************************************selects*/ 
        displayIngs = (y,z)=>{
            if(z=='1'){
                ...
                abc=document.getElementById(y).parentNode.parentNode.parentNode.childNodes[1].innerHTML
            }
            document.getElementById(y).innerHTML='<i class="fa fa-spinner fa-spin"></i>'
            Cook.getRecipe(y).then(function(data) {
                if(data.data.success){
                    rec = data.data.recipe
                    content ='<span><img src="assets/img/recipes/'+rec.Recipe+'.png" height="25px" width="25px">'+rec.Recipe+'</span><br><hr class="style14"><br><table class="table table-striped" style="    background-color: transparent">'
                    var ing = [];
                    var qt = [];
                    if (rec.Ingredient1 != "") {
                        ing.push(rec.Ingredient1);
                    ...
                    for (var i = 0; i < qt.length; i++) {
                        if(qt[i] != ""){
                            content+='<tr><td><img src="assets/img/recipes/'+ing[i]+'.png" height="25px" width="25px"></td><td>'+ing[i]+'</td><td>'+parseInt(qt[i]*abc)+'</td></tr>'
                        }
                    }
                    content+="</table>"
                    arr = rec.Buff.split(",");
                    ...
                    if(document.getElementById(y)){
                        document.getElementById(y).innerHTML=y+" is not a cooked ingredient"
                    }
                }
            })
        }
        ingToRecipe = function(y){
            ...
            Cook.getRecipe(y).then(function(data2) {                
                Cook.getRecipesFromIngredient(y).then(function(data3){
                    if(data3.data.success){
                        if(typeof data3.data.recipes !== 'undefined' && data3.data.recipes.length > 0){
                            usedIn.innerHTML = '<br><hr class="style14"><br><h3><b>'+y+'</b> <img src="assets/img/recipes/'+y+'.png" height="30px" width="30px"> is also used in: </h3><br>';
                            tab=document.createElement("table")
                            tab.setAttribute('class', 'table table-hover table-bordered table-responsive table-striped');
                            ...
                            tab.setAttribute('id',"possibles")
                                            ...
                                            for (var i = 0; i < data3.data.recipes.length; i++) {
                                                ...
                                                td.innerHTML='<div class="ing" onmouseover="displayIngs(&quot;'+rec.Recipe+'&quot;,2)" onclick="ingToRecipe(&quot;'+rec.Recipe+'&quot;)" style="cursor:pointer"><img src="assets/img/recipes/'+rec.Recipe+'.png" height="30px" width="30px"><div id="'+rec.Recipe+'" class="tooltiptext"></div></div>'
                                                tr.appendChild(td)
                                                ...
                                                for (var j = 0; j < ing.length; j++) {
                                                    ...
                                                    td2.innerHTML='<span class="row"><span  style="margin: 0 auto;">'+qt[j]+'&emsp;'+ing[j]+'</span</span><span class="row" onclick="ingToRecipe(&quot;'+ing[j]+'&quot;)" style="cursor:pointer;"><img style="margin: 0 auto" src="assets/img/recipes/'+ing[j]+'.png" height="25px" width="25px"></span>'
                                                    tr2.appendChild(td2)
                                                }
        ...
        $(".chars").on('select2:select', function(e) {
            ...
            Cook.getAllLifeInfos().then(function(data) {
                Cook.getLifeInfos(app.charLvl).then(function(data2) {
...
/******************************************Yes/no*/
    ...
    ////////////////////********************time canape yes/no *****************/////////
        if($window.localStorage['canape']){
            app.cnp = $window.localStorage['canape']
        }else{
            app.cnp = 0
            app.canape=false
            $window.localStorage['canape'] = 0
        }
        app.canapeF = function(val) {
            if (val == 'yes') {
                app.cnp = -2
                app.canape=true
                $window.localStorage['canape']=-2
            } else {
                app.canape = false
                app.cnp = 0
                $window.localStorage['canape']=0
        ...
        app.checkcanape0 = function(){
            if(app.cnp == "-2" ){
                return true
            }
            else return false
        }
        app.checkcanape1 = function(){
            if(app.cnp == "0" ){
                return true
            }
            else return false
    ...
    ////////////////////********************time food*****************/////////
        if($window.localStorage['fd3']){
            app.fd3 = $window.localStorage['fd3']
        }else{
            app.fd3 = 0
            $window.localStorage['fd3'] = 0
        }
        app.food3F = function(val) {
            app.fd3 = val
            $window.localStorage['fd3'] = val
        }
    ...
    ////////////////////********************time clothes*****************/////////
        if($window.localStorage['ck']){
            app.ck = $window.localStorage['ck']
        }else{
            app.ck = 0
            $window.localStorage['ck'] = 0
        }
        app.ckclothesF = function(val) {
            app.ck = val
            $window.localStorage['ck'] = val
        }
    ...
    ////////////////////********************time alchemy stone *****************/////////
        if($window.localStorage['as']){
            app.as = $window.localStorage['as']
            app.as2 = $window.localStorage['as']
        }else{
            app.as = 0
            app.as2 = 0
            $window.localStorage['as'] = 0
        }
        app.asF = function(val) {
            app.as = val
            app.as2 = parseFloat(val).toFixed(1)
            $window.localStorage['as']=val
        }
    ...
    ////////////////////********************time cooking utensil *****************/////////
        if($window.localStorage['cu']){
            app.cu = $window.localStorage['cu']
        }else{
            app.cu = 0
            $window.localStorage['cu'] = 0
        }
        app.cuF = function(val){
            app.cu = val
            $window.localStorage['cu']=val
        }
...
/*******************************************/
    ////////////////////********************Useful functs *****************/////////
                    ...
                    if (rec[i].Recipe == b) {
                        ...
                        if (w == 0) {
                            if (rec[i].Ingredient1 != "") {
                                ...
                                td.innerHTML += "<img src='assets/img/recipes/" + i1 + ".png' width='25px' height='25px'/>" + i1;
                                tr.appendChild(td);
                                if (rec[i].Ingredient2 != "") {
                                    td = document.createElement('td');
                                    td.setAttribute('style', 'width:' + wid + '%;vertical-align:middle;text-align:center');
                                    td.innerHTML = "<img src='assets/img/recipes/" + i2 + ".png' width='25px' height='25px'/>" + i2;
                                    tr.appendChild(td);
                                    if (rec[i].Ingredient3 != "") {
                                        td = document.createElement('td');
                                        td.setAttribute('style', 'width:' + wid + '%;vertical-align:middle;text-align:center');
                                        td.innerHTML = "<td><img src='assets/img/recipes/" + i3 + ".png' width='25px' height='25px'/>" + i3;
                                        tr.appendChild(td);
                                        if (rec[i].Ingredient4 != "") {
                                            td = document.createElement('td');
                                            td.setAttribute('style', 'width:' + wid + '%;vertical-align:middle;text-align:center');
                                            td.innerHTML = "<td><img src='assets/img/recipes/" + i4 + ".png' width='25px' height='25px'/>" + i4;
                                            tr.appendChild(td);
                                            if (rec[i].Ingredient5 != "") {
                                                td = document.createElement('td');
                                                td.setAttribute('style', 'width:' + wid + '%;vertical-align:middle;text-align:center');
                                                td.innerHTML = "<td><img src='assets/img/recipes/" + i5 + ".png' width='25px' height='25px'/>" + i5;
                                                tr.appendChild(td);
        ...
        app.stopTime = ()=>{
            ...
            app.timeck = "Cooking Session Stopped"
            app.amount2 = "0"
            app.progOk2 = false            
            app.prog = 0
            app.progOk3 = false
        ...
        experince = function(regData) {
            ...
            app.err_msg = ""
                    ...
                    Dashboard.getexp().then(function(data) {
                        ...
                        Cook.getRecipe(regData.recipe).then(function(data2) {
                            ...
                            var tata=[];
                            var ff=m_xp;
                            Cook.getLifeInfos(app.rank2).then(function(data3) {
        ...
        cook = function(regData) {
            ...
            app.err_msg = ""
	                    ...
	                    Dashboard.getexp().then(function(data) {
	                        ...
	                        Cook.getRecipe(app.recipe).then(function(data2) {
	                            ...
	                            Cook.getLifeInfos(app.rank).then(function(data3) {
	                                Cook.getLifeInfos(life).then(function(data4) {
	                                    if (parseInt(data3.data.recipeLevel.Xp) >= parseInt(data4.data.recipeLevel.Xp)) {
	                                        ...
	                                        Cook.getAllLifeInfos().then(function(data5) {
    ...
    ////////////////////******************** Time *****************/////////
        ...
        time=function(regData){
            ...
            pearl = parseFloat(regData.canape)
            cloth = parseFloat(regData.cookingclothes)
            utensil = parseFloat(regData.cu)
            food = parseFloat(regData.food3)
            all=parseFloat(stone+pearl+cloth+utensil+food+10).toFixed(1)
            if(all >= 1){
                app.ctime = all
            }
            ...
            app.amount = regData.amount
            app.timereduced = parseFloat(stone+pearl+cloth+utensil+food).toFixed(1)
            ...
            function loopme() {
                ...
                if(app.timeck <=0){
                    $interval.cancel(interval)                    
                    app.timeck = "Cooking Session Ended"                    
                    app.amount2 = "0"
                    app.progOk2 = false
                    app.progOk3 = false
                    app.disabledt=false
            ...
            sendnotif = $timeout(function(){
                ...
                User.sendNotification(message).then(function(data){
                    if(!data.data.success){
                        ...
                        alert('Cooking session complete')
                    }
                });                
    ...
    ///////////////////---------------------Byproduct--------------------//////////
        ...
        turn=(regData)=>{
            ...
            if(app.beerMin >= 2) app.beermin = numberWithCommas(Math.floor(app.beerMin/2))
            if(app.beerMax >= 2) app.beermax = numberWithCommas(Math.ceil(app.beerMax/2))
            ...
            if(app.milkMin >= 3) app.milkmin = numberWithCommas(Math.floor(app.milkMin/3))
            if(app.milkMax >= 3) app.milkmax = numberWithCommas(Math.ceil(app.milkMax/3))
        ...
        app.dwmif=function(c){
            if(c>=2){
                app.dwmi=numberWithCommas(Math.floor(c/2))
            }else{
                app.dwmi=null
            }
        }
        ...
        app.ssdf=function(c){
            if(c>=3){
                app.ssd=numberWithCommas(Math.floor(c/3))
            }else{
                app.ssd=null
            }
        }
